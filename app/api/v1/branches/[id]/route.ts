...
import { getBranch, updateBranch, softDeleteBranch } from '@/lib/services/branch.service'
import { ServiceError } from '@/lib/services/auth.service'

/**
 * GET /api/v1/branches/[id]
 * Returns the branch when found.
 ...
 * Updates allowed fields on an existing branch. Administrador only.
 * Returns 200 { branch: BranchDTO } on success.
 * Returns 401 if unauthenticated.
 * Returns 403 if the caller is not Administrador.
 * Returns 404 if the branch does not exist or is soft-deleted.
 ...
 * Soft-deletes a branch (sets deletedAt = now). Administrador only.
 * Returns 200 { success: true } on success.
 * Returns 401 if unauthenticated.
 * Returns 403 if the caller is not Administrador.
 * Returns 404 if the branch does not exist or is already soft-deleted.
 * Returns 409 { error: 'branch_has_active_assignments' } when active employees are assigned.
 */
